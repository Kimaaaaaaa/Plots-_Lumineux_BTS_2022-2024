  ...
  void onConnect(BLEServer* pServer ){
    deviceConnected = true;
     ...
     void onWrite(BLECharacteristic *pCharacteristic) override {
        ...
        if (value.length() > 0) {
            // Ici, vous pouvez traiter la valeur reçue, par exemple en changeant la couleur d'une LED
            Serial.println("Valeur reçue : ");
            for (int i = 0; i < value.length(); i++)
                Serial.print(value[i]);
            
            Serial.println();
            // Convertissez la valeur en données utilisables selon votre cas d'usage.
        }
    }
...
void setup() {
  ...
  BLEService *pServicePlot = pServer->createService(CHARACTERISTIC_BATTERY_UUID_TX);
